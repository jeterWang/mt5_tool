    ...
    try:
        ...
        if not symbol_info.visible:
            if not mt5.symbol_select(symbol, True):
                logger.error(f"选择交易品种失败: {symbol}")
                return None
...
def cancel_order(ticket: int) -> bool:
    ...
    try:
        ...
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.error(f"撤销订单失败，错误代码：{result.retcode}")
            return False
...
def place_pending_order(
    ...
    try:
        ...
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.error(f"挂单失败，错误代码：{result.retcode}，描述：{result.comment}")
            return None
...
def place_order_with_key_level_sl(
    ...
    try:
        ...
        result = mt5.order_send(request)
        if result.retcode != mt5.TRADE_RETCODE_DONE:
            logger.error(f"下单失败，错误代码：{result.retcode}，描述：{result.comment}")
            return None
