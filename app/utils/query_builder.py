...
class QueryBuilder:
    ...
    def where_date_range(self, field: str, start_date: Union[str, date, datetime], 
...
class TradeQueryBuilder(QueryBuilder):
    ...
    def trade_count_table(self):
        """使用trade_count表"""
        return self.from_table("trade_count")
    
    def risk_events_table(self):
        """使用risk_events表"""
        return self.from_table("risk_events")
    
    def today_trades(self, trading_day: str):
        """查询今日交易"""
        return self.trade_count_table().where_date_equals("date", trading_day)
    
    def recent_trades(self, days: int = 7):
        """查询最近几天的交易"""
        return (self.trade_count_table()
                .order_by_desc("date")
                .limit(days))
    
    def recent_risk_events(self, days: int = 7):
        """查询最近的风控事件"""
        return (self.risk_events_table()
...
def example_usage():
    ...
    # 基础SELECT查询
    query, params = (select("date", "count")
    ...
    # INSERT查询
    query, params = insert_into("trade_count", {
        "date": "2024-01-15",
        "count": 5
    })
    # print(f"插入: {query}")
    ...
    query, params = (builder
                    .where_equals("date", "2024-01-15")
                    .build_update("trade_count", {"count": 10}))
    # print(f"更新: {query}")
    # print(f"参数: {params}")
