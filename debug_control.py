...
def toggle_debug_prints(enable_debug=False):
    ...
    logger.info(f"{'开启' if enable_debug else '关闭'}调试日志...")
...
def add_debug_config():
    ...
    try:
        ...
        if "DEBUG" not in config:
            ...
            logger.info("已添加调试配置到 config.json")
        else:
            logger.info("调试配置已存在")
            
    except Exception as e:
        logger.error(f"错误: 添加调试配置失败: {e}")

if __name__ == "__main__":
    import sys
    
    logger.info("MT5交易系统调试日志控制工具")
    logger.info("=" * 40)
    ...
    if len(sys.argv) > 1:
        ...
        logger.info("使用方法:")
        logger.info("  python debug_control.py on   # 开启调试日志")
        logger.info("  python debug_control.py off  # 关闭调试日志")
        logger.info()
