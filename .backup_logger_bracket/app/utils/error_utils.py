...
def error_context(operation: str, 
    ...
    try:
        ...
        logger.error(f"操作失败 [{error_id}]: {operation}")
        raise
            ...
            for attempt in range(config.max_attempts):
                try:
                    ...
                    if attempt < config.max_attempts - 1:
                        logger.warning(f"重试 {func.__name__} (第{attempt + 1}次): {str(e)}")
                        import time
                        time.sleep(delay)
                        delay *= config.backoff_factor
                    else:
                        logger.error(f"{func.__name__} 重试失败，已达最大重试次数")
...
def handle_mt5_connection_error(e: Exception, operation: str = "MT5操作") -> bool:
    ...
    logger.error(f"MT5连接错误 [{error_id}]: {str(e)}"))
    return False
...
def handle_trading_error(e: Exception, symbol: str = "", operation: str = "交易操作") -> bool:
    ...
    logger.error(f"交易错误 [{error_id}]: {str(e)}"))
    return False
...
def handle_db_error(e: Exception, operation: str = "数据库操作") -> Any:
    ...
    logger.error(f"数据库错误 [{error_id}]: {str(e)}"))
    return None
...
def handle_config_error(e: Exception, operation: str = "配置操作") -> bool:
    ...
    logger.error(f"配置错误 [{error_id}]: {str(e)}"))
    return False
