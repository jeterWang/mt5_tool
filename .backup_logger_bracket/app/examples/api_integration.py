    ...
    def _test_connection_status(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"✗ 连接状态请求失败: {response.status_code}"))
        except Exception as e:
            logger.error(f"✗ 连接状态测试失败: {e}"))
        ...
        try:
            ...
            if adapter:
                ...
                if adapter.initialize():
                    ...
                    logger.error("⚠ API适配器初始化失败"))
                
                adapter.cleanup()
            
        except Exception as e:
            logger.error(f"现有系统集成演示错误: {e}")
            logger.error(f"✗ 现有系统集成演示失败: {e}"))
